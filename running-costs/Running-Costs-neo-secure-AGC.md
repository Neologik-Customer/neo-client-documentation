# Running Costs: neo-secure with AGC

This document sets out the expected monthly Azure running cost of a Neologik platform environment deployed with the **neo-secure with Application Gateway for Containers** infrastructure profile in the UK South region. It explains what drives the cost and how spend is monitored, so that there are no surprises on the bill.

## Summary

This variant of neo-secure keeps the same private network layout, with Azure Bastion and a self hosted runner VM, and uses Application Gateway for Containers (AGC) with the web application firewall as its ingress. The profile also includes a jumpbox VM.
...
| NAT gateway (400 GB), load balancer, public IP addresses | £56 | £56 |
| Application Gateway for Containers with WAF | £188 | £188 |
...
| **Fixed infrastructure total** | **£1,149** | **£1,203** |
...
| Networking | NAT gateway for outbound traffic (400 GB processed a month, as measured), Standard load balancer, 2 static public IP addresses |
